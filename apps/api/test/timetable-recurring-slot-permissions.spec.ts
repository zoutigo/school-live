...
function makeSlotExceptionEntity(overrides: Record<string, unknown> = {}) {
...
describe("createSlotException — referent seulement", () => {
  const EXISTING_SLOT = makeSlotEntity();
  ...
  beforeEach(() => {
    prisma.classTimetableSlot.findFirst.mockResolvedValue(EXISTING_SLOT);
    prisma.classTimetableSlotException.upsert.mockImplementation(
      async ({ create }: { create: Record<string, unknown> }) =>
        makeSlotExceptionEntity({
          occurrenceDate: create.occurrenceDate,
          type: create.type,
          subjectId: create.subjectId ?? null,
          teacherUserId: create.teacherUserId ?? null,
          startMinute: create.startMinute ?? null,
          endMinute: create.endMinute ?? null,
          room: create.room ?? null,
          reason: create.reason ?? null,
          subject: create.subjectId ? { id: "sub-1", name: "Maths" } : null,
          teacherUser: create.teacherUserId
            ? {
                id: "teacher-albert",
                firstName: "Albert",
                lastName: "Teacher",
...
describe("updateSlotException — referent seulement", () => {
  const EXISTING_EXCEPTION = makeSlotExceptionEntity();

  beforeEach(() => {
    prisma.classTimetableSlotException.findFirst.mockResolvedValue(
      EXISTING_EXCEPTION,
    );
    prisma.classTimetableSlotException.update.mockImplementation(
      async ({ data }: { data: Record<string, unknown> }) =>
        makeSlotExceptionEntity({
          occurrenceDate:
            data.occurrenceDate ?? EXISTING_EXCEPTION.occurrenceDate,
          type: data.type ?? EXISTING_EXCEPTION.type,
          subjectId:
            data.subjectId === undefined
              ? EXISTING_EXCEPTION.subjectId
              : data.subjectId,
          teacherUserId:
            data.teacherUserId === undefined
              ? EXISTING_EXCEPTION.teacherUserId
              : data.teacherUserId,
          startMinute:
            data.startMinute === undefined
              ? EXISTING_EXCEPTION.startMinute
              : data.startMinute,
          endMinute:
            data.endMinute === undefined
              ? EXISTING_EXCEPTION.endMinute
              : data.endMinute,
          room: data.room === undefined ? EXISTING_EXCEPTION.room : data.room,
          reason:
            data.reason === undefined ? EXISTING_EXCEPTION.reason : data.reason,
          subject:
            data.subjectId === undefined || data.subjectId === null
              ? null
              : { id: "sub-1", name: "Maths" },
          teacherUser:
            data.teacherUserId === undefined || data.teacherUserId === null
              ? null
              : {
                  id: "teacher-albert",
                  firstName: "Albert",
