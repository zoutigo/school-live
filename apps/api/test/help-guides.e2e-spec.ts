...
describe("Help guides API e2e", () => {
  ...
  it("retourne un guide et son plan pour un parent", async () => {
    ...
    expect(current.body?.guide).toBeTruthy();
    expect(current.body?.resolvedAudience).toBe("PARENT");
    expect(current.body?.canManage).toBe(false);
    ...
    expect(Array.isArray(plan.body?.items)).toBe(true);
    expect((plan.body?.items as unknown[]).length).toBeGreaterThanOrEqual(2);
  });

  it("autorise un super admin à lister les guides admin", async () => {
    const result = await apiJson("/api/help-guides/admin/guides", {
  ...
  it("désactive la gestion quand le role actif est scolaire", async () => {
    ...
    const admin = await apiJson("/api/help-guides/admin/guides", {
      headers: { authorization: `Bearer ${hybridToken}` },
    });
