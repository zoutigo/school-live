...
<html lang="fr">
  ...
  async sendRoomStatusChangeNotification(payload: RoomStatusChangeMailPayload) {
    ...
    await transporter.sendMail({
      from,
      to: payload.to,
      subject: `Scolive - ${payload.title} (${payload.roomName})`,
      text: [
        `Bonjour ${payload.recipientFirstName},`,
        "",
        payload.summary,
...
<html lang="fr">
  ...
  async sendHomeworkCreatedNotification(payload: HomeworkCreatedMailPayload) {
    ...
    await transporter.sendMail({
      from,
      to: payload.to,
      subject: `Scolive - Nouveau devoir: ${payload.title}`,
      text: [
        `Bonjour ${payload.recipientFirstName},`,
        "",
        `Un nouveau devoir a ete publie pour la classe ${payload.className}.`,
        `Matiere: ${payload.subjectName}`,
        `Titre: ${payload.title}`,
        `A faire pour le: ${payload.expectedAtLabel}`,
        `Enseignant: ${payload.authorFullName}`,
        "",
        `Consulter le portail: ${schoolUrl}`,
      ].join("\n"),
      html: `
...
<html lang="fr">
          ...
          <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width:600px;background:#FFFFFF;border:1px solid #E3E6E8;border-radius:10px;overflow:hidden;">
            ...
            <tr>
              <td style="padding:24px;">
                ...
                <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#4A4A4A;">
                  Un nouveau devoir a ete publie pour la classe <strong>${payload.className}</strong>.
                </p>
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 16px;background:#F8F9FA;border:1px solid #E3E6E8;border-radius:8px;">
                  <tr><td style="padding:12px 16px;font-size:14px;color:#4A4A4A;">
                    <div><strong>Matiere :</strong> ${payload.subjectName}</div>
                    <div><strong>Titre :</strong> ${payload.title}</div>
                    <div><strong>A faire pour le :</strong> ${payload.expectedAtLabel}</div>
                    <div><strong>Enseignant :</strong> ${payload.authorFullName}</div>
                  </td></tr>
                </table>
                <a href="${schoolUrl}" style="display:inline-block;background:#0A62BF;color:#FFFFFF;text-decoration:none;padding:10px 16px;border-radius:8px;font-size:14px;font-weight:600;">
                  Ouvrir le portail
  ...
  async sendGradePublishedNotification(payload: GradePublishedMailPayload) {
    ...
    const webUrl =
      this.configService.get<string>("WEB_URL") ?? "http://localhost:3000";
    const notesUrl = payload.schoolSlug
      ? `${webUrl}/schools/${payload.schoolSlug}/notes`
      : `${webUrl}/`;

    const studentFullName =
      `${payload.studentFirstName} ${payload.studentLastName}`.trim();
    const noteTypeLabel = payload.isFinalExam
      ? "Composition de fin de séquence"
      : "Devoir / Evaluation";
    const countsLabel = payload.countsForAverage
      ? "Comptabilisée dans la moyenne"
      : "Evaluation formative (non comptabilisée dans la moyenne de séquence)";
    ...
    await transporter.sendMail({
      from,
      to: payload.to,
      subject: `Scolive - Nouvelle note publiée · ${payload.subjectName}`,
      text: [
        `Bonjour ${payload.recipientFirstName},`,
        "",
        `Une nouvelle note a été publiée pour ${studentFullName}.`,
        `Classe : ${payload.className}`,
        `Matière : ${payload.subjectName}`,
        `Évaluation : ${payload.evaluationTitle}`,
        `Séquence : ${payload.sequenceLabel}`,
        `Type : ${noteTypeLabel}`,
        `Statut : ${countsLabel}`,
        "",
        `Consulter les notes : ${notesUrl}`,
      ].join("\n"),
      html: `
<!doctype html>
<html lang="fr">
          ...
          <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width:600px;background:#FFFFFF;border:1px solid #E3E6E8;border-radius:10px;overflow:hidden;">
            ...
            <tr>
              <td style="padding:24px;">
                ...
                <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#4A4A4A;">
                  Une nouvelle note a été publiée pour <strong>${studentFullName}</strong>.
                </p>
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 16px;background:#F8F9FA;border:1px solid #E3E6E8;border-radius:8px;">
                  <tr><td style="padding:14px 16px;font-size:14px;color:#4A4A4A;line-height:1.7;">
                    <div><strong>Classe :</strong> ${payload.className}</div>
                    <div><strong>Matière :</strong> ${payload.subjectName}</div>
                    <div><strong>Évaluation :</strong> ${payload.evaluationTitle}</div>
                    <div><strong>Séquence :</strong> ${payload.sequenceLabel}</div>
                    <div><strong>Type :</strong> ${noteTypeLabel}</div>
                    <div style="margin-top:8px;padding:6px 10px;border-radius:6px;display:inline-block;background:${payload.countsForAverage ? "#E6F4EA" : "#FFF3E0"};color:${payload.countsForAverage ? "#1B5E20" : "#E65100"};font-size:13px;font-weight:600;">
                      ${countsLabel}
                    </div>
                  </td></tr>
                </table>
