...
type CreateMessageResult =
  ...
  | { broadcast: true; schools: number; messages: MessageDetailResponse[] };
...
function senderLabel(
  ...
  const base = sender
    ? `${sender.lastName} ${sender.firstName}`.trim() || sender.email
    : "Moi";
  return school ? `${base} · ${school.name}` : base;
}
...
function mapListItemToUi(item: MessageListItemResponse): MessagingMessage {
  return {
    id: item.id,
    folder: item.folder,
    sender: senderLabel(item.sender, item.school),
...
export async function listPlatformMessages(params: ListMessagesParams) {
  ...
  const response = await fetch(
    `${API_URL}/admin/messages?${query.toString()}`,
    {
      credentials: "include",
    },
  );
...
export async function getPlatformMessage(messageId: string) {
...
export async function markPlatformMessageRead(messageId: string, read = true) {
...
export async function createPlatformMessage(payload: CreateMessagePayload) {
  const csrfToken = getCsrfTokenCookie();
  if (!csrfToken) {
    throw new Error("CSRF_TOKEN_MISSING");
  }
...
export async function getPlatformMessagesUnreadCount() {
  const response = await fetch(`${API_URL}/admin/messages/unread-count`, {
    credentials: "include",
  });
  if (!response.ok) {
    return 0;
...
export async function archivePlatformMessage(
  messageId: string,
  archived: boolean,
) {
  const csrfToken = getCsrfTokenCookie();
  ...
  const response = await fetch(
    `${API_URL}/admin/messages/${messageId}/archive`,
    {
      method: "PATCH",
      credentials: "include",
      headers: {
...
export async function uploadPlatformMessagingInlineImage(file: File) {
  const csrfToken = getCsrfTokenCookie();
  if (!csrfToken) {
    throw new Error("CSRF_TOKEN_MISSING");
  }
  ...
  const response = await fetch(
    `${API_URL}/admin/messages/uploads/inline-image`,
    {
      method: "POST",
      credentials: "include",
      headers: {
